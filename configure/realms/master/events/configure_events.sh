...
echo " - configuration des evenements d'administration"
# Active l'enregistrement des evenements d'admin dans la base de donnees keycloak
$KCADM update events/config -r ${REALM_NAME} -s adminEventsEnabled=true -s adminEventsDetailsEnabled=true

echo " - configuration des evenements de login"
# Active l'enregistrement des evenement de login dans la base keycloak 
# et defini l'expiration en heure  de ces evenements (suppression de a base) 
$KCADM update events/config -r ${REALM_NAME} -s eventsEnabled=true -s eventsExpiration=259200

# Defini les types des evenement de login à enregistrer enregitrer dans la base.
